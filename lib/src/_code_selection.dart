...
class _MobileSelectionToolbar extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final List<Widget> resultChildren =
        _getAdaptiveButtons(context, items).toList();
    Widget? toolbar;
    if (Platform.isIOS) {
  ...
  Iterable<Widget> _getAdaptiveButtons(
      BuildContext context, List<ToolbarMenuItem> buttonItems) {
    if (Platform.isAndroid) {
      ...
      for (int i = 0; i < buttonItems.length; i++) {
        ...
        buttons.add(TextSelectionToolbarTextButton(
          padding:
              TextSelectionToolbarTextButton.getPadding(i, buttonItems.length),
          onPressed: () {
